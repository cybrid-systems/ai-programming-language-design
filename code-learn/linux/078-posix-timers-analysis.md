...
## 3. hrtimer_start——底层定时器启动

（`kernel/time/hrtimer.c` — doom-lsp 确认）

```c
// POSIX 定时器最终通过 hrtimer 实现：
void hrtimer_start(struct hrtimer *timer, ktime_t tim, const enum hrtimer_mode mode)
{
    struct hrtimer_clock_base *base;

    // 1. 获取 clock base（根据 timer->base->clockid）
    //    CLOCK_MONOTONIC → base = &cpu_base->clock_base[HRTIMER_BASE_MONOTONIC]
    //    CLOCK_REALTIME  → base = &cpu_base->clock_base[HRTIMER_BASE_REALTIME]
    ...
    //    → 重新编程硬件时钟（APIC timer 或 ktime）
}
```

## 4. POSIX 定时器 vs Linux 低精度定时器

| 特性 | POSIX 定时器 | timer_list（定时器轮） |
|------|-------------|---------------------|
| 实现 | hrtimer（红黑树） | timer wheel（分级桶） |
| 精度 | 纳秒 | jiffies（1-10ms） |
| 通知 | 信号/线程 | 回调函数 |
| 时钟源 | HRTIMER_BASE_* | jiffies（基于 tick） |
| 适用 | 纳秒级精确定时 | 超时/重试/延迟 |
