...
```c
// mm/swapfile.c — 分配 swap slot（当前使用 folio_alloc_swap）
int folio_alloc_swap(struct folio *folio)
{
    swp_entry_t entry;

    entry = folio_alloc_swap_entry(folio);
    if (!entry.val)
        return 0;

    // 添加到 swap cache
    if (folio_add_swap_cache(folio, entry, GFP_KERNEL, NULL) < 0) {
        folio_free_swap_entry(folio, entry);
        return 0;
    }
...
kswapd / direct reclaim 选择要换出的页面
  │
  ├─ shrink_folio_list → folio_alloc_swap(folio)
  │    │
  │    ├─ folio_alloc_swap(folio) → 分配 swap entry
  │    ├─ swapcache 索引更新
...
| 操作 | 延迟 | 说明 |
|------|------|------|
| folio_alloc_swap | ~100ns | slot 分配 |
...
Swap 提供内存扩展能力。folio_alloc_swap 分配 entry，folio_add_swap_cache 标记新页面，__swap_writepage 写磁盘，do_swap_page 换入。簇分配优化顺序 I/O，zswap 提供压缩替代。
...
memcg 可以单独限制每个 cgroup 的 swap 使用：

```bash
/sys/fs/cgroup/<group>/memory.swap.max
# 设置 cgroup 的 swap 上限
# 0 表示禁用 swap

# 当 cgroup 的 swap 使用超限时
# 触发 memcg swap OOM
# 在 cgroup 内杀进程
```
...
- mm/swapfile.c — swap 区域管理
- mm/swap_state.c — swap 缓存
- mm/page_io.c — swap I/O
- Documentation/admin-guide/swap.rst
...
The swap subsystem provides virtual memory expansion. It manages swap areas through swap_info_struct structures. The swap_map byte array tracks per-slot reference counts. Cluster allocation groups adjacent slots for sequential I/O. folio_alloc_swap and do_swap_page manage the page transfer between memory and swap devices.
