...
{
    list_for_each_entry_rcu(e, &audit_filter_list[EXIT], list) {
...
5 种过滤类型：USER → TASK → ENTRY → WATCH → EXIT
...
{
    struct audit_buffer *ab = this_cpu_read(audit_buffer_cpu);
    if (ab) { this_cpu_write(audit_buffer_cpu, NULL); return ab; }
    ab = kmalloc(...); ab->skb = alloc_skb(AUDIT_BUFSIZ);
    return ab;
}
```
...
audit_log_start 分配 audit_buffer 时首先尝试 per-CPU 缓存，缓存未命中时才 kmalloc + alloc_skb。该优化减少了高频审计事件下的分配开销。


过滤引擎支持 5 种过滤类型：USER 针对用户空间事件、TASK 在进程创建时检查、ENTRY 在 syscall 入口检查、WATCH 针对文件系统、EXIT 在 syscall 退出时检查。EXIT 是最常用的过滤点。
