...
```c
// drivers/base/devres.c:154-158（内部函数）
...
**doom-lsp 确认**：`devres_add` 在 `devres.c:277`。`list_add_tail` 将新节点加到链表尾部，释放时从头部往前遍历——实现**逆序释放**。

### 2.3 find_dr — 查找资源

```c
// drivers/base/devres.c:285-300
...
**doom-lsp 确认**：`release_nodes` 在 `devres.c:540`。`list_for_each_entry_safe_reverse` 确保逆序释放——最后注册的资源最先释放。
...
| 943 | `devm_kmalloc()` (devm_kzalloc is inline wrapper in devres.h) |
| 118 | `dr_node_release()` — 分层回调 |
| 125 | `dr_node_free()` |
