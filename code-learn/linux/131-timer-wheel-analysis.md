...
```c
struct timer_list {
    struct hlist_node   entry;      // L13 — 哈希链表节点（挂在 timer_base.vectors[] 的 bucket 中）
    unsigned long       expires;    // L14 — 到期时间（jiffies 值）
    void                (*function)(struct timer_list *); // L15 — 回调函数
    u32                 flags;      // L16 — 标志位（BASEMASK, DEFERRABLE, IRQSAFE, PINNED 等）
#ifdef CONFIG_LOCKDEP
    struct lockdep_map  lockdep_map;// L19 — lockdep 依赖映射
#endif
};
```
...
```c
struct timer_base {
    raw_spinlock_t      lock;               // L251 — 保护 timer_base 的自旋锁
    struct timer_list   *running_timer;     // L252 — 当前正在执行的回调的定时器
    unsigned long       clk;                // L257 — 当前定时器轮的时钟（jiffies + 1 offset）
    unsigned long       next_expiry;        // L258 — 下一个到期时间（jiffies 值）
    unsigned int        cpu;                // L259 — 所属 CPU
    bool                next_expiry_recalc; // L260 — 是否需要重新计算 next_expiry
    bool                is_idle;            // L261 — CPU 是否 idle
    bool                timers_pending;     // L262 — 是否有挂起的定时器
    DECLARE_BITMAP(pending_map, WHEEL_SIZE); // L263 — 各 bucket 是否非空（快速扫描）
    struct hlist_head   vectors[WHEEL_SIZE]; // L264 — 所有 bucket（WHEEL_SIZE = 8×64 = 512）
} ____cacheline_aligned;   // L265
```
