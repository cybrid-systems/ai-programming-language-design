...
**doom-lsp 确认**：`pick_next_rt_entity` 在 `rt.c:1676`。`sched_find_first_bit()` 是架构相关的高效位扫描（通常使用 `__ffs` 或 `BSF` 指令），复杂度 O(1)。
...
**doom-lsp 确认**：`rt_sched_class` 的完整方法表在 `rt.c:2595`。共 19 个回调函数覆盖 RT 调度器的完整生命周期。
...
**doom-lsp 确认**：`push_rt_task()` 在 `rt.c:1953`。`push_rt_tasks()`（`rt.c:2071`）是一个 `while` 循环，持续推送直到无可推送任务。`pick_next_pushable_task()`（`rt.c:1840`）遍历 `plist_head` 并跳过 `task_on_cpu()` 的任务（proxy-execution 下可能在另一个 CPU 上运行）。
...
| `kernel/sched/rt.c` | 1650 | `set_next_task_rt()` |
| `kernel/sched/rt.c` | 1768 | `find_lowest_rq()` |
| `kernel/sched/rt.c` | 1953 | `push_rt_task()` |
| `kernel/sched/rt.c` | 2254 | `pull_rt_task()` |
| `kernel/sched/rt.c` | 2534 | `task_tick_rt()` |
| `kernel/sched/rt.c` | 2595 | `DEFINE_SCHED_CLASS(rt)` |
