...
ssize_t generic_perform_write(struct kiocb *iocb, struct iov_iter *i)
{
    ...
    do {
        // ——— 1. 写入前准备 ———
        status = a_ops->write_begin(iocb, mapping, pos, len,
                                     &folio, &fsdata);
        // ext4_write_begin:
        //   ├─ iocb 传入替代 file
...
## 4. 🔥 写回路径——`writepages` + folio 遍历

在 Linux 7.0-rc1 中，`mm/page-writeback.c` 不再有 `write_cache_pages`。写回由文件系统自身的 `a_ops->writepages()` 实现，内部使用 XArray 标记遍历脏 folio。
...
```
            ...
            └────────┬────────┘
                     │
               a_ops->writepages / a_ops->writepage
                     │
                     ▼
            ┌─────────────────┐
            │  WRITEBACK      │  ★ 正在写回磁盘
...
| `mm/page-writeback.c` | 脏页回写核心（`write_cache_pages` 已移除，文件系统实现 `a_ops->writepages`） | — |
