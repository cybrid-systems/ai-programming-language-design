...
struct futex_q {
    ...
    futex_wake_fn           *wake;          // 唤醒回调函数（普通/PI 不同）
    void                    *wake_data;     // 唤醒回调数据
    union futex_key         key;            // futex 的哈希键
    struct futex_pi_state   *pi_state;      // PI 状态（PI futex 使用）
    struct rt_mutex_waiter  *rt_waiter;     // rt_mutex 等待者存储
    union futex_key         *requeue_pi_key;// requeue_pi 目标 futex 键
    u32                     bitset;         // 位掩码唤醒
    atomic_t                requeue_state;  // requeue_pi 状态
    bool                    drop_hb_ref;    // 是否释放额外 hash bucket 引用
};
```
...
struct futex_hash_bucket {
    ...
    struct plist_head       chain;          // futex_q 的优先级链表
    struct futex_private_hash *priv;        // 私有哈希（futex2）
...
struct futex_pi_state {
    struct list_head        list;           // 拥有的 pi_state 列表
    struct rt_mutex_base    pi_mutex;       // 底层的 RT mutex
    struct task_struct      *owner;         // 当前持有者
    refcount_t              refcount;       // 引用计数
    union futex_key         key;            // 对应的 futex 键
};
```
