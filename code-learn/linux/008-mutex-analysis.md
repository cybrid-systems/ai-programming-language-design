...
```c
// include/linux/mutex_types.h — doom-lsp 确认
struct mutex {
    atomic_long_t               owner;          // 锁所有者 + flags
    raw_spinlock_t              wait_lock;      // 等待队列自旋锁
#ifdef CONFIG_MUTEX_SPIN_ON_OWNER
    struct optimistic_spin_queue osq;           // Spinner MCS 锁
#endif
    struct mutex_waiter __rcu   *first_waiter;  // 等待队列头的 waiter
...
> 注：`struct mutex` 实际通过 `context_lock_struct(mutex)` 宏在 `mutex_types.h` 中定义，展开后即如上所示。等待者不再使用 `struct list_head wait_list`，而是使用 `first_waiter` 指针指向 FIFO 链表的第一个 waiter，waiter 通过自身的 `struct list_head list` 互相链接。

**`owner` 字段的位编码**（`kernel/locking/mutex.c`）：

```c
// owner 字段的低 3 位编码锁状态（kernel/locking/mutex.h:33）
#define MUTEX_FLAG_WAITERS     0x01    // bit 0: 有等待者
#define MUTEX_FLAG_HANDOFF     0x02    // bit 1: 锁正在交接
#define MUTEX_FLAG_PICKUP      0x04    // bit 2: 等待者可获取锁
...
```c
// kernel/locking/mutex.h:15 — 内部使用
struct mutex_waiter {
    struct list_head    list;           // 与其它 waiter 的 list 互相链接
    struct task_struct  *task;          // 等待的进程
    struct ww_acquire_ctx *ww_ctx;      // w/w mutex 使用
#ifdef CONFIG_DEBUG_MUTEXES
    void                *magic;
#endif
};
...
```
当前线程插入到 mutex 的 first_waiter 链表（按 FIFO 顺序）
  → set_current_state(TASK_UNINTERRUPTIBLE)
  → schedule()                ← 让出 CPU
  → 被唤醒后，从 first_waiter 链表头部竞争锁
  → 获取成功 → 从 first_waiter 链表移除
```
...
当锁的持有者释放锁时，如果有等待者在 `first_waiter` 链表中，它必须将锁**直接交接**给下一个等待者，而不是将 `owner` 设为 0 让所有等待者竞争。这称为 **handoff**：

```
mutex_unlock() 时检测到 first_waiter 非空：
  │
  ├─ 设置 MUTEX_FLAG_HANDOFF（告诉等待者：锁正在交接）
  ├─ 在 first_waiter 链表中选择第一个 waiter
  ├─ 将该 waiter 从列表移除
...
```c
// kernel/locking/mutex.c:208 — doom-lsp 确认
                               ...
                               struct mutex_waiter *pos)
{
    ...
    if (first) {
        list_add_tail(&waiter->list, &first->list);  // FIFO 尾部插入
        if (pos == first)
    ...
    __must_hold(&lock->wait_lock)
{
    ...
    } else if (lock->first_waiter == waiter) {
        lock->first_waiter = list_first_entry(&waiter->list, struct mutex_waiter, list);
    }
...
**FIFO 公平性**：等待者按到达顺序插入 `first_waiter` 链表尾部。每次解锁时只唤醒队首的 waiter，严格保证了公平性，避免了写者饥饿（writer starvation）。
  ...
  └─ __mutex_trylock_fast(lock)            @ mutex.c:153
      ...
      └─ 失败 → __mutex_lock_slowpath(lock) @ mutex.c:???
            ...
            └─ __mutex_lock(lock, TASK_UNINTERRUPTIBLE, 0, NULL)
                  ...
                  │   ├─ __mutex_add_waiter(lock, waiter, NULL)  ← 插入 first_waiter 链表
...
| 文件 | 内容 | doom-lsp 符号数 |
|------|------|----------------|
| `include/linux/mutex.h` | 声明 + inline | **73 个** |
| `include/linux/mutex_types.h` | `struct mutex` 定义 | — |
| `kernel/locking/mutex.h` | `mutex_waiter` + 位编码定义 | — |
