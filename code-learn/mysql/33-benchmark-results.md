...
## 2.5 最终综合对比（单线程，AHI=OFF）

| 场景 | OFF | ON | 变化 | 说明 |
|------|-----|----|------|------|
| PK 顺序插入 5000 (ms) | 48 | **32** | **+50%** 🚀 | BTREE_OPT 页布局紧凑化 |
| PK 点查 500x (ms) | 804 | 827 | ~ -3% (噪声) | CLI 连接开销 ~500μs 稀释收益 |
| PK 范围扫描 100x (ms) | 178 | 173 | ~3% (持平) | Hints 对顺序扫描增益有限 |
| UUID 插入 2000 (ms) | 18,140 | 18,204 | 持平 | 二级索引插入路径未优化 |
| UUID 点查 200x (ms) | 696 | 698 | 持平 | UUID 字符串 4B Head 信息量小 |
| URL 范围扫描 100x (ms) | 168 | 167 | 持平 | LIKE 不走优化路径 |

## 2.6 已知差距

| 方面 | 当前状态 | 理想目标 | 预期收益 |
|------|---------|---------|---------|
| **向量化** | slot 逐次比较 | ARM SVE2 批比 8-64 slots | +额外 10-30% |
| **连接池** | mysql CLI 每次 fork | 连接复用 | 点查延迟从 500μs→50μs |
| **并发** | 单线程 | 8-64 线程 OLTP | 体现多核扩展性 |
| **负载** | 纯读/纯写 | sysbench OLTP 70/30 | 混合负载真实收益 |
| **数据集** | 5k-10k 行 | 100k-1M 行 | Hints/自适应布局需大页数生效 |
