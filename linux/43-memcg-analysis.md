...
The memcg subsystem tracks memory usage per cgroup. Each page allocation is charged to the allocating process's cgroup through the try_charge path. The page_counter structure maintains usage counters that are checked hierarchically from the leaf cgroup up to root. When a limit is exceeded, the kernel either triggers reclaim within the cgroup or initiates memcg OOM.
