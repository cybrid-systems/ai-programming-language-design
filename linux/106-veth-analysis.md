...
struct veth_rq {                             // 接收队列
    ...
    struct napi_struct napi;                 // 普通 NAPI 实例
    struct net_device *dev;
    struct bpf_prog __rcu *xdp_prog;         // XDP BPF 程序
    struct xdp_mem_info xdp_mem;             // XDP 内存
    struct veth_rq_stats *stats;             // 队列统计
    bool rx_notify_masked;
    struct sk_buff_head xdp_ring;            // XDP skb 队列
    struct xdp_rxq_info xdp_rxq;
    struct page_pool *page_pool;             // 页面池
};

struct veth_rq_stats {                       // per-queue 统计
...
// 1. xdp_prog = rcu_dereference(rcv_priv->xdp_prog);
...
// static int veth_poll(struct napi_struct *napi, int budget)
// {
//     struct veth_rq *rq = container_of(napi, struct veth_rq, napi);
//     int done = 0;
//
//     while (done < budget) {
//         skb = __skb_dequeue(&rq->xdp_ring);
//         if (!skb) break;
//         napi_gro_receive(napi, skb);
//         done++;
//     }
