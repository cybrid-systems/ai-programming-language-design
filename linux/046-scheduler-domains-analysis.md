...
**doom-lsp 确认**：sched_domain 的分配在 `__sdt_alloc()`（`topology.c:2380`）中通过 `kzalloc_node(sizeof(struct sched_domain) + cpumask_size(), ...)` 完成，span 的物理空间紧接着结构体末尾。`sched_domain_span()` 宏（`topology.h:108`）通过 `(void *)sd + sizeof(*sd)` 直接计算 span 地址。
...
**doom-lsp 确认**：`sched_group_capacity` 在 `__sdt_alloc()`（`topology.c:2416`）中分配：`kzalloc_node(sizeof(struct sched_group_capacity) + cpumask_size(), GFP_KERNEL, cpu_to_node(j))`。`sgc->id` 初始化为 `j`（CPU 编号）。`sgc->cpumask[]` 存储的是 group balance mask，由 `group_balance_mask()` 宏（`sched.h:2216`）访问。
...
**系统默认**：`static struct root_domain def_root_domain`（`topology.c:490`）是默认根域，包含所有 CPU。
...
```c
// include/linux/sched/topology.h:147-157
...
在 `sd_init()`（`topology.c:1973`）中，拓扑描述表的 flags（来自 `cpu_smt_flags()` 等）被合并到 C 风格的结构体初始化中：
...
`asym_cpu_capacity_scan()`（`topology.c:1718`）遍历所有可能的 CPU，按 `arch_scale_cpu_capacity()` 分桶：

```c
// topology.c:1718-1745
...
**`asym_cpu_capacity_classify()`**（`topology.c:1622`）在构建每个域时调用，检测该域是否覆盖多种 CPU 容量，返回所需标志：
...
`build_sched_domains()` 是域构建的**主函数**（`topology.c:2706`），分为五个阶段：
...
| `include/linux/sched/topology.h` | 63 | `struct sched_domain_shared` |
| `include/linux/sched/topology.h` | 147 | `struct sched_domain_topology_level` |
...
| `kernel/sched/topology.c` | 723 | `cpu_attach_domain()` |
| `kernel/sched/topology.c` | 1309 | `build_sched_groups()` |
| `kernel/sched/topology.c` | 1150 | `build_overlap_sched_groups()` |
| `kernel/sched/topology.c` | 1622 | `asym_cpu_capacity_classify()` |
| `kernel/sched/topology.c` | 2706 | `build_sched_domains()` |
