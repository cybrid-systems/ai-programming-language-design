...
| **AST 层** | C++26 | Trees that Grow + Concepts | 早期处理、静态反射、源位置保留 | [aura_memory_pool.md](./aura_memory_pool.md) |
| **AuraIR 层** | C++26 | 自定义 SSA-like IR | 优化核心、代码生成入口 | — |
| **AuraQueryEngine** | C++26 | 倒排索引 + eDSL | 子树查询、变换、AI 接口 | [query.md](./aura_query.md) |
| **Compiler Service** | C++26 | 长驻进程 | 编译 + 执行 + 热更新 + 状态 | [aura_caas.md](./aura_caas.md) |
...
**内存管理**：所有 AST 节点通过 pmr 内存池 (`ASTArena`) 分配，支持 O(1) 整树回收和零碎片的 bump 分配。详细设计见 [aura_memory_pool.md](./aura_memory_pool.md)。
...
**完整实现方案见 [aura_caas.md](./aura_caas.md)**。本文仅列核心接口与架构分解。

Compiler Service 基于 pmr 内存池（[aura_memory_pool.md](./aura_memory_pool.md)）构建，每个编译请求使用独立 `ASTArena`，支持增量编译、多租户隔离和 AI Agent 实时代码变异。
...
| 架构设计 | 多篇分阶段 | 整合版 → 本文档 |
| 内存池 | [aura_memory_pool.md](./aura_memory_pool.md) | `src/core/arena.ixx` |
| CaaS 集成 | [aura_caas.md](./aura_caas.md) | `src/compiler/service.ixx` |
